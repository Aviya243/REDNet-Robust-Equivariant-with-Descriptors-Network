...
def train(conf, train_data, model, phase, validation_data=None, test_data=None, fabri=None):
    ...
    best_epoch = 0
    converge_time = -1
    begin_time = time() ; batch_time_100 = time()
    ...
    for epoch in range(conf["resuming_epoch"] + 1, num_of_epochs):
        ...
        if fabric.global_rank == 0:
            ...
            if epoch % 50 == 0:
                # print(f'{fabric.global_rank}:{epoch} Train Loss: {mean_train_loss:.2f}')
                # print(f"Time for last 100 epochs: {time()-batch_time_100:.1f} seconds")
                VRAM_per = memory_track_utils.get_gpu_memory() ; RAM_per = memory_track_utils.get_ram_memory()
                stats = {
                    "Rank": [fabric.global_rank],
                    "Epoch": [epoch],
                    "Train Loss": [mean_train_loss.item()],
                    "Time [s]": [time()-batch_time_100],
                    "VRAM [%]": [VRAM_per],
                    "RAM [%]": [RAM_per]
                }
                df_stats = pd.DataFrame(stats)
                df_stats[['Train Loss', 'Time [s]', 'VRAM [%]', 'RAM [%]']] = df_stats[['Train Loss', 'Time [s]', 'VRAM [%]', 'RAM [%]']].round(3)
