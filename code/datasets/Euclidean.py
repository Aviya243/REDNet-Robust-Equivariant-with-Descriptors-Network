...
from utils import memory_track_utils
import pandas as pd
...
def get_raw_data(conf, scan, phase):
    ...
    Args:
        ...
        scan (str): Name of the scene to load (without file extension)
        phase (Phases): Current phase (optimization/training/validation/testing/fine-tuning)
    ...
    Returns:
        ...
        Added Returned Property:
        S (torch.Tensor): Descriptors (e.g., SIFT) for each point [m, n, 128]
    """
    ...
    use_descriptors = conf.get_bool('dataset.use_descriptors', default=False)

    # print(f"Used Dataset: {dataset_name}")
    # print(f"Loading from: {dataset_path}")

    VRAM_per = memory_track_utils.get_gpu_memory() ; RAM_per = memory_track_utils.get_ram_memory()
    stats = {
        "Dataset" : [dataset_name],
        "Scene" : [scan],
        "Use Descriptors" : [use_descriptors],
        "VRAM [%]": [VRAM_per],
        "RAM [%]": [RAM_per]
    }
    df_stats = pd.DataFrame(stats)
    df_stats[['VRAM [%]', 'RAM [%]']] = df_stats[['VRAM [%]', 'RAM [%]']].round(3)
    print(df_stats.to_string(index=False)) ; print('\n')
    ...
    outliers_np = dataset.get('outliers2', np.zeros((M_np.shape[0] // 2, M_np.shape[1]))) # [m, n], boolean mask
    ...
    outliers = torch.from_numpy(outliers_np).float() # [m, n], 0/1 (floats) mask
    outliers_mask = outliers.clone()

    if use_descriptors:
        descriptor_path = os.path.join(path_utils.path_to_datasets(dataset_name), f'{scan}'.split()[0]+' SIFT Descriptors.pt')
        S = torch.load(descriptor_path)
        S = S.detach().to(M.device)
    else:
        S = None
    ...
    if phase is Phases.FINE_TUNE and output_mode == 3:
        _, valid_cam_indices = dataset_utils.check_if_M_connected(M, thr=1, return_largest_component=True) # A list of camera indices (subset of [m])
        double_cam_indices = [j for i in [[idx * 2, idx * 2 + 1] for idx in valid_cam_indices] for j in i] # Corresponding indices fo M (as M is [2m, n])
    ...
    return M, Ns, Ps_gt, outliers, dict_info, names_list, M_original, S
